...
# Windows'ta çift tıklayınca terminal açık kalsın
if os.name == 'nt' and not sys.stdin.isatty():
    # Eğer terminal yoksa (çift tıklama ile açıldıysa), yeni terminal aç
    import subprocess
    import ctypes
    
    # Yeni terminal penceresi aç
    if sys.executable.endswith('pythonw.exe'):
        # pythonw.exe ile çalıştırıldıysa, python.exe ile yeniden başlat
        subprocess.Popen([sys.executable.replace('pythonw.exe', 'python.exe'), __file__])
        sys.exit(0)
    else:
        # Terminal açık tutmak için
        try:
            # Windows'ta console'u göster
...
if __name__ == '__main__':
    ...
    # Windows'ta çift tıklayınca terminal açık kalsın ve tarayıcı otomatik açılsın
    if os.name == 'nt':  # Windows
        ...
        print("=" * 50)
        print("  Akademik Dergi Yonetim Sistemi")
        print("=" * 50)
        print("\nUygulama baslatiliyor...")
        print("Tarayici otomatik olarak acilacak...")
        print("\nUygulamayi durdurmak icin Ctrl+C tuslarina basin")
        print("=" * 50)
        print()
